## How many terminal columns a rune occupies.
##
## `ansi.displayWidth` and everything layered on it — truncation, padding, box
## drawing, the renderer's clipping — are only correct if this is. A rune that
## is measured as one column but drawn as two pushes the rest of the line right,
## and because the renderer's cursor arithmetic assumes no line wrapping, a wide
## rune at the right-hand edge wraps and desynchronises the whole frame rather
## than just that line.
##
## The rules are the usual `wcwidth` ones:
##
## * 0 for combining marks, format characters and other zero-width codepoints
## * 2 for East Asian Wide and Fullwidth codepoints, and for emoji that have
##   Emoji_Presentation (terminals draw those in two cells)
## * 1 for everything else, including East Asian *Ambiguous*
##
## Ambiguous is the important default: box-drawing characters, block elements,
## braille and `…` are all Ambiguous, and a terminal in the usual western
## locale draws them in one cell. Treating them as wide would break every
## bordered panel.
##
## What this does not do, all of which needs grapheme clustering rather than a
## per-rune width:
##
## * emoji ZWJ sequences — `👨‍👩‍👧` measures 6, not 2
## * regional-indicator flag pairs — `🇬🇧` measures 2, not 1 (which is what most
##   terminals draw, so this one is usually right by accident)
## * variation selector 16 — `❤️` measures 1, because U+2764 has text
##   presentation by default and the following U+FE0F is zero-width. Getting
##   this right means looking ahead one rune and carrying an Emoji property
##   table, which is a `displayWidth` change rather than a `runeWidth` one
## * base + combining sequences are measured correctly only because the marks
##   are 0; the cluster is never treated as a unit

import std/unicode

# Tables are sorted and non-overlapping; `tests/tansi.nim` asserts both, which
# is what catches a transcription slip when a range is added.

const ZeroWidthRanges = [
  ## Nonspacing marks (Mn), enclosing marks (Me) and format characters (Cf),
  ## plus Hangul Jamo medial vowels and final consonants, which combine onto
  ## the preceding leading consonant.
  ...
  (0x0859, 0x085B), (0x0898, 0x089F), (0x08CA, 0x0902), (0x093A, 0x093A),
...
const WideRanges = [
  ## East Asian Wide (W) and Fullwidth (F), plus the emoji blocks terminals
  ## draw in two cells. East Asian Ambiguous is deliberately absent — see the
  ## module docs.
  ...
  (0x1AFF0, 0x1AFFE), (0x1B000, 0x1B152), (0x1B164, 0x1B167),
...
static:
  # The binary search below silently misses codepoints if a table is out of
  # order or a range overlaps its neighbour, and neither shows up as anything
  # more obvious than a slightly wrong column count. Check it at compile time
  # so a mistyped range fails the build rather than a layout.
  for table in [@ZeroWidthRanges, @WideRanges]:
    for i, (lo, hi) in table:
      doAssert lo <= hi, "reversed range at index " & $i & ": " & $lo
      if i > 0:
        doAssert table[i - 1][1] < lo,
          "unsorted or overlapping range at index " & $i & ": " & $lo

func contains(ranges: openArray[(int, int)], cp: int): bool =
...
func runeWidth*(r: Rune): int =
  ...
  if ZeroWidthRanges.contains(cp): return 0
  if WideRanges.contains(cp): return 2
  1

func runeWidth*(c: char): int =

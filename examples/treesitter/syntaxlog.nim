## `examples/gitlog.nim` with tree-sitter syntax highlighting laid over the
## commit diff — the same two axes on the same bytes that
## `examples/treesitter/syntaxdiff.nim` composes, arriving here from the other
## side.
##
## **The added idea: a patch is a picture of a file, and only the file knows
## what its colours are.** `gitdiff` already parses a unified patch into rows,
## so `syntaxdiff` had somewhere to hang a foreground. `gitlog` deliberately
## does not: its diff pane holds the bytes `git show` produced, coloured by
## looking at the first character of each line, which is all a reader of a
## patch-as-text can know. Syntax colour needs the two things that reading
## costs nothing to throw away and everything to recover — *which file* a line
## belongs to and *which line of it* — so this variant parses the patch
## (`parsePatch`_) rather than prefix-matching it, and the hunk headers become
## `PatchRow.oldNo` / `newNo` instead of a `@@` to colour blue.
##
## What the numbers are for: highlighting a hunk *fragment* gives confidently
## wrong colours (that is `snippets.nim`'s `maxErrorRatio` case), so the blobs
## are pulled whole — `git show <sha>:<path>` and `git show <sha>^:<path>` — and
## a row indexes into the side it came from. A `+` line reads the new blob, a
## `-` line the old one, a context line either. The parse is then checked
## against the patch line it is meant to be (`sourceFor`_): a source line that
## is not the line on screen colours the wrong runes at exactly the right width,
## which is the one failure no dimensional assertion here can see.
##
## Three things follow, and each is a place this diverges from `gitlog`:
##
## * **The blobs are a second load, behind the patch.** `gitlog`'s debounce
##   exists because holding `j` must not run a `git show` per keypress; a commit
##   touching forty files would queue eighty more subprocesses behind the one
##   the reader is waiting for. So `hlCmd`_ is issued *after* the patch lands,
##   under a generation counter of its own, and the colours arrive a beat later
##   with the pane rebuilt where it stands rather than scrolled back to the top.
## * **The palette is background-only**, as in `syntaxdiff`: `gitlog`'s
##   foreground-only `ckDefault` fallback would fight the grammar, so the tints
##   are always mixed from a ground — the real background when the terminal
##   answers `OSC 11`, a dark stand-in when it will not.
## * **`c` and `s` are mutually exclusive, and that is the honest answer.**
##   `gitlog`'s `c` takes git's own `--color=always` output, which is bytes that
##   already carry their colours; there is nowhere in them for a capture to go.
##   So with git's colours on the syntax layer is simply not applied, and the
##   pane footer says which of the three is drawing.
##
## Everything else is `gitlog`: the four levels of key dispatch, the paged
## history with its generation guard, the debounce, the filter, the wheel
## against `regions`, `execCmd` to git's pager and to `$EDITOR`, and the
## containment answer in the header. `.nim` has no grammar, so this repository's
## own commits render through the plain fallback — which `sourceFor` returning
## nil reproduces exactly, so the fallback is the same code rather than a branch.
##
##   nimble snippets            # builds everything under examples/treesitter/
##   ./bin/syntaxlog [path]
##   ./bin/syntaxlog --selftest
...
const
  ...
  MaxDiffLines = 4000
  MaxHlFiles = 40
    ## Files in one commit whose blobs are fetched and parsed. A commit touching
    ## more than this is a merge or a vendor drop, where the reader is scanning
    ## the shape rather than reading the code — and the cost is two `git show`
    ## subprocesses and two parses per file. Past the cap the rows fall through
    ## to the plain colouring, which is what a file with no grammar gets anyway.
  MaxHlBytes = 1_000_000
    ## The largest blob worth parsing. A generated file is the case this exists
    ## for: nobody reads its diff, and it is exactly the file that is a megabyte.
...
type
  ...
  RowKind = enum
    rkMeta                       ## `diff --git`, `index`, `---`, `+++`, `\ No newline`
    rkHunk                       ## `@@ … @@`
    rkContext, rkAdd, rkDel

  PatchRow = object
    ## One line of the patch carrying the two things a syntax layer needs and a
    ## string of bytes does not: which file it belongs to, and where in that
    ## file's *source* it sits. Both come from the headers — `diff --git` and
    ## `@@` — and are gone by the time the pane holds strings, which is the whole
    ## reason this example parses the patch where `gitlog` prefix-matches it.
    kind: RowKind
    file: int                    ## index into `patchFiles`, -1 before the first
    oldNo, newNo: int            ## 1-based, 0 where that side has no line
    text: string                 ## the patch line verbatim, sign included

  PatchFile = object
    ## The two paths a `diff --git` stanza names, `""` where the side does not
    ## exist (`/dev/null` — a file added or deleted by this commit).
    ##
    ## Taken from the `---` / `+++` lines rather than from `diff --git`, which is
    ## ambiguous by construction: it separates the two paths with a space and
    ## does not quote them, so `a/my file b/my file` has three readings. The
    ## corner cut here, as in `gitdiff`: a path git *did* quote is not unquoted.
    oldPath, newPath: string

  Blob = object
    ## One side of one file: the source, its per-line spans, and the styles
    ## resolved for *its* grammar.
    ##
    ## A `ref` because the model is a value and `rebuildDiff` reads a blob once
    ## per patch line — copying a megabyte of source per row is the difference
    ## between a rebuild and a stall. Per file rather than shared for the reason
    ## `snippets.nim` gives: capture ids are per grammar, so one resolved seq
    ## used across two languages colours one of them by the other's numbering.
    src: string
    hl: Highlights
    styles: seq[Style]
  ...
  FileHl = object
    oldSide, newSide: BlobRef    ## nil: no such side, no grammar, or too big
  ...
  HlMsg = ref object of Msg
    ## Every blob the selected commit touched, parsed. The `Tree`s are dropped —
    ## a tree is not copyable and the model is a value — so only the spans and
    ## the resolved styles survive the crossing.
    gen: int
    sha: string
    files: seq[FileHl]
  ...
  Model = object
    ...
    syntax: bool                 ## lay the grammar's colours over the patch
    diffOf: string               ## the sha the diff pane currently holds
    patch: string                ## the patch as it arrived, for a re-render
    patchFiles: seq[PatchFile]
    rows: seq[PatchRow]
    hl: seq[FileHl]              ## parallel to `patchFiles`
    hlOf: string                 ## the sha `hl` belongs to, "" for none
    hlGen: int
    frame: int
    animate: bool
    status: string
...
proc showBlob(root, rev, path: string): string =
  ## `git show <rev>:<path>`, and "" for every way that can fail — a side that
  ## is `/dev/null`, a path that did not exist at that revision, a root commit
  ## with no `^`. None of them is an error here: a file with one side missing is
  ## a file this commit added or deleted, which is the ordinary case.
  if path.len == 0: return ""
  try: git(root, ["show", rev & ":" & path])
  except CatchableError: ""

proc parseBlob(path, source: string): BlobRef =
  ## Highlight one blob, or return nil — which is the same answer for "no
  ## grammar", "not there" and "too big", because the renderer has one fallback
  ## for all three and nothing to say about which it took.
  if source.len == 0 or source.len > MaxHlBytes: return nil
  let lang = detectLanguage(path = path, source = source)
  if lang == nil: return nil
  let h = newHighlighter(lang).highlight(source)
  BlobRef(src: source, hl: h,
          styles: defaultSyntaxStyles().compile(h.captureNames))

proc hlCmd(root, sha: string, files: seq[PatchFile], gen: int): Cmd =
  ## Both blobs for every file the commit touched, parsed.
  ##
  ## A load of its own rather than more work inside `diffCmd`_, on the same
  ## reasoning the debounce is there for: this is two subprocesses and two
  ## parses *per file*, and putting it in front of the patch would make the pane
  ## wait on forty files to show the first one. The patch is what the reader
  ## asked for; the colours are what arrives next.
  ##
  ## `sha^` is the first parent, which is the side `git show` diffed against —
  ## so the hunk line numbers land. A root commit has no `^`, and `showBlob`
  ## answers "" for it, which is the same as a file that was added.
  result = proc (): Msg =
    var res = newSeq[FileHl](files.len)
    for i in 0 ..< min(files.len, MaxHlFiles):
      res[i].oldSide = parseBlob(files[i].oldPath,
                                 showBlob(root, sha & "^", files[i].oldPath))
      res[i].newSide = parseBlob(files[i].newPath,
                                 showBlob(root, sha, files[i].newPath))
    HlMsg(gen: gen, sha: sha, files: res)
...
proc hunkStarts(header: string): (int, int) =
  ## The two line numbers a `@@ -12,7 +9,8 @@` header opens with. The counts are
  ## not needed — the rows themselves say how far the hunk runs — and the comma
  ## is optional (`@@ -1 +1 @@` for a one-line hunk), so each side is read as
  ## digits up to a comma or a space.
  proc num(s: string, i: var int): int =
    while i < s.len and s[i] notin {'0' .. '9'}: i.inc
...
proc patchPath(field: string): string =
  ## The path out of a `--- a/src/x.nim` or `+++ b/src/x.nim` line. `/dev/null`
  ## is the side that does not exist, and comes back as "".
  ##
  ## The corner cut, named because it is the same one `gitdiff` cuts: a path
  ## containing a space or a quote is quoted and C-escaped by git, and is not
  ## unquoted here. It costs a file its colours, never a wrong picture — the
  ## `git show` of a mangled path fails and both sides come back nil.
  var p = field.strip
  if p == "/dev/null": return ""
  let tab = p.find('\t')          # git appends a timestamp on some paths
  if tab >= 0: p = p[0 ..< tab]
  if p.len > 2 and (p.startsWith("a/") or p.startsWith("b/")): p = p[2 .. ^1]
  p

proc parsePatch(patch: string): (seq[PatchFile], seq[PatchRow]) =
  ## A unified patch as rows that know where they came from.
  ##
  ## The one rule that is easy to get wrong: **inside a hunk, a line starting
  ## with `---` is a removed line**, not a file header. Removing a line that
  ## reads `--` produces exactly that, and treating it as a header both loses
  ## the line and re-points every row after it at the wrong file. So the header
  ## forms are recognised only *before* the first `@@` of a stanza, which is
  ## where git puts them, and `inHunk` is the whole of that distinction.
  var
    files: seq[PatchFile]
    rows: seq[PatchRow]
    cur = -1
  ...
  for line in patch.splitLines:
    if rows.len >= MaxDiffLines: break
    template meta() = rows.add PatchRow(kind: rkMeta, file: cur, text: line)
    if line.startsWith("diff --git "):
      ...
      inHunk = true
      rows.add PatchRow(kind: rkHunk, file: cur, text: line)
    elif not inHunk and line.startsWith("--- "):
      if cur >= 0: files[cur].oldPath = patchPath(line[4 .. ^1])
      meta()
    elif not inHunk and line.startsWith("+++ "):
      if cur >= 0: files[cur].newPath = patchPath(line[4 .. ^1])
      meta()
    elif inHunk and line.len > 0 and line[0] == '+':
      rows.add PatchRow(kind: rkAdd, file: cur, newNo: newNo, text: line)
      newNo.inc
    elif inHunk and line.len > 0 and line[0] == '-':
      rows.add PatchRow(kind: rkDel, file: cur, oldNo: oldNo, text: line)
      oldNo.inc
    elif inHunk and line.len > 0 and line[0] == ' ':
      rows.add PatchRow(kind: rkContext, file: cur,
                        oldNo: oldNo, newNo: newNo, text: line)
      oldNo.inc
      newNo.inc
    else:
      # `\ No newline at end of file`, `Binary files … differ`, `index …`, and
      # the blank line `splitLines` leaves after the final newline. None of them
      # is a line of the file, so none of them advances a counter.
      meta()
  (files, rows)
...
proc scheduleHl(m: var Model): Cmd =
  ## Ask for the selected commit's blobs, once its patch is on screen.
  ##
  ## Guarded by a counter of its own rather than by `diffGen`, because the two
  ## loads are at different rates on purpose: the patch is debounced against the
  ## cursor and this is issued from the patch's own arrival, so a reader holding
  ## `j` runs neither. The sha rides along as well as the generation, since the
  ## answer is only usable against the commit it was asked about.
  if not m.syntax or m.gitColour: return nil
  if m.diffOf.len == 0 or m.patchFiles.len == 0: return nil
  m.hlGen.inc
  hlCmd(m.root, m.diffOf, m.patchFiles, m.hlGen)
...
proc applyBackground(m: var Model, bg: Color) =
  ...
  ## case rather than an error, and the answer to it here is not `gitlog`'s.
  ## That one falls back to a *foreground-only* design, the sign carried by the
  ## colour of the text; with a grammar colouring the same text that is two
  ## palettes fighting over one channel. So every style below is a background,
  ## always mixed from a ground — the real one when the terminal gave it, a dark
  ## stand-in when it did not — and the foreground belongs to the capture.
  m.bg = bg
  let dark = bg.kind == ckDefault or bg.luminance < 0.5
  m.theme = derive(hex"#5ad1c0", dark = dark)
  let ground = if bg.kind != ckDefault: bg
               elif dark: hex"#161616" else: hex"#f4f4f4"
  # 0.32, where `gitlog` mixes 0.20. A tint is doing more work here than it does
  # there: with the sign carried by a foreground too, a hint of colour is enough
  # to disambiguate a `+` from a `-`, and here it is the *only* thing saying the
  # line changed at all — so it has to read as a band across the row rather than
  # as a shade of the pane. What stops it going further is not taste but the
  # dimmest capture in the syntax palette: a comment is `#7f848e`, and against
  # these two grounds it sits at 3.0:1 on the tint at 0.32, 2.6:1 at 0.38 and
  # 2.3:1 at 0.45 — which is a comment inside a changed line fading out, and a
  # comment is exactly what a changed line is likely to contain. The tint is
  # therefore bounded by the faintest thing that has to stay legible on it, and
  # anything that brightens these has to check that number again.
  m.addStyle = Style().bg(lerp(ground, m.theme.success, 0.32))
  m.delStyle = Style().bg(lerp(ground, m.theme.error, 0.32))
  m.hunkStyle = Style().fg(m.theme.info).bg(lerp(ground, m.theme.info, 0.10))
  # A tint under the whole diff pane, so it reads as a sheet of paper laid on
  # the terminal. It is also what puts `renderOver` on the hot path: every line
  # in this pane already carries escapes — git's own with `c` on, the grammar's
  # otherwise — and a plain `render` would end this background at the first
  # reset in the line and leave the rest of the row bare.
  m.paneStyle = Style().bg(lerp(ground, m.theme.accent, 0.05))
...
proc tabbed(text: string, col: var int): string =
  ## `text` with its tabs expanded against the column it starts at.
  ##
  ## Needed because the two colouring modes would otherwise disagree about
  ## indentation. `Spans.add` flattens a tab to a single space — it is a control
  ## character, and an *action* to the terminal, so a measured line may not
  ## contain one — while `TextArea.reflow` expands it to `DefaultTabStop`. A
  ## plain patch reaches the pane as a string and gets the second; a highlighted
  ## one is built out of runs and would get the first, and a tab-indented file
  ## would move eight columns left the moment its colours arrived.
  ##
  ## The column carries across runs, since a capture edge can fall between a tab
  ## and the stop it is measured to.
  if '\t' notin text:
    col += displayWidth(text)
    return text
  result = newStringOfCap(text.len + 8)
  for r in text.runes:
    if r == Rune('\t'):
      let n = DefaultTabStop - (col mod DefaultTabStop)
      result.add spaces(n)
      col += n
    else:
      result.add $r
...
proc sourceFor(m: Model, row: PatchRow): (BlobRef, int) =
  ## The blob this row's text is a copy of, and its 1-based line in it — or
  ## `(nil, 0)`, which is the fallback for every reason at once: the layer is
  ## off, git is doing the colours, the highlight belongs to another commit, the
  ## file was past `MaxHlFiles`, the language is unknown.
  ##
  ## A `+` reads the new blob and a `-` the old one; a context line is in both
  ## and takes whichever exists, since one side of a rename or a mode change may
  ## have no grammar while the other does.
  ##
  ## The last clause is the one worth keeping: **the source line is checked
  ## against the patch line it claims to be.** Everything above is arithmetic on
  ## numbers that came from a hunk header, and if any of it is off by one the
  ## result is a line coloured by the grammar of the line *next to it* — the
  ## right number of columns, plausible colours, and wrong. That is invisible to
  ## every dimensional assertion in this file, so it is checked directly, and a
  ## mismatch falls through to the plain rendering rather than being drawn.
  if not m.syntax or m.gitColour or m.hlOf.len == 0 or m.hlOf != m.diffOf:
    return (nil, 0)
  if row.file < 0 or row.file >= m.hl.len: return (nil, 0)
  let f = m.hl[row.file]
  var (b, no) =
    case row.kind
    of rkAdd: (f.newSide, row.newNo)
    of rkDel: (f.oldSide, row.oldNo)
    of rkContext:
      if f.newSide != nil: (f.newSide, row.newNo) else: (f.oldSide, row.oldNo)
    else: (BlobRef(nil), 0)
  if b == nil or no <= 0 or no > b.hl.lineRanges.len: return (nil, 0)
  let r = b.hl.lineRanges[no - 1]
  let text = if r.b >= r.a: b.src[r.a .. r.b] else: ""
  if text != row.text[1 .. ^1]: return (nil, 0)
  (b, no)

proc anyGrammar(m: Model): bool =
  ## Whether the highlight on hand found a grammar for anything in this commit.
  ## Worth a footer state of its own: a commit of `.nim` files and a commit
  ## whose colours have not arrived yet are the same picture on screen, and the
  ## difference between "wait" and "there is nothing coming" is the whole of
  ## what the reader wants to know.
  for f in m.hl:
    if f.oldSide != nil or f.newSide != nil: return true

proc codeSpans(m: Model, row: PatchRow, base: Style): Spans =
  ## One patch line: the sign, then the code — spliced at every capture edge
  ## when the grammar has something to say about it, one run when it does not.
  ##
  ## The capture supplies a foreground and `base` a background, and `merge` is
  ## what lets both be true of one run: a colour that is `ckDefault` in the
  ## overlay means *not specified* and leaves the other alone, so the diff tint
  ## lands without touching the colour the grammar chose. That is the whole of
  ## the composition, and it is why the palette above had to give up its
  ## foregrounds.
  let sign = if row.text.len > 0: row.text[0 .. 0] else: ""
  var col = displayWidth(sign)
  result.add(sign, base)
  let (b, no) = m.sourceFor(row)
  if b == nil:
    if row.text.len > 1: result.add(tabbed(row.text[1 .. ^1], col), base)
    return
  let r = b.hl.lineRanges[no - 1]
  var pos = r.a
  template emit(a, z: int, st: Style) =
    if a < z: result.add(tabbed(b.src[a ..< z], col), merge(st, base))
  for sp in b.hl.lines[no - 1]:
    emit(pos, sp.startByte, Style())
    emit(sp.startByte, sp.endByte,
         (if sp.capture >= 0 and sp.capture < b.styles.len: b.styles[sp.capture]
          else: Style()))
    pos = sp.endByte
  emit(pos, r.b + 1, Style())

proc rebuildDiff(m: var Model) =
  ## The pane's lines, from the parsed patch.
  ##
  ## Called on a load, on `c`, on `s`, and when the highlight arrives — never
  ## per frame. `view` is a pure function of the model, so this is where the
  ## strings have to be made; and it deliberately does not touch the scroll,
  ## because the commonest caller is the highlight landing under a reader who is
  ## already part way down the patch.
  ##
  ...
  if m.patch.strip.len == 0:
    lines = @["", "  (no diff — a merge commit shows none by default)"]
  elif m.gitColour:
    lines = plainPatch(m.patch)
  else:
    for row in m.rows:
      case row.kind
      of rkHunk: lines.add m.hunkStyle.render(row.text)
      of rkAdd: lines.add m.codeSpans(row, m.addStyle).render()
      of rkDel: lines.add m.codeSpans(row, m.delStyle).render()
      of rkContext: lines.add m.codeSpans(row, m.paneStyle).render()
      of rkMeta:
        if row.text.len == 0: lines.add ""
        elif row.text.startsWith("+++") or row.text.startsWith("---"):
          lines.add t.mutedStyle.bold.render(row.text)
        else: lines.add t.mutedStyle.render(row.text)
    if m.rows.len >= MaxDiffLines:
      lines.add t.warnStyle.render(&"… truncated at {MaxDiffLines} lines")
  m.diff.setLines lines
  # `displayWidth`, not `len`: every one of these lines is mostly escape
  # sequences, which cost no columns.
  m.longest = 0
  for ln in lines: m.longest = max(m.longest, displayWidth(ln))
...
proc onKey(m: var Model, k: KeyMsg): Cmd =
  ...
  if k.matches("q", "ctrl+c"): return quitCmd()
    ...
    # The syntax layer, on and off — which is how to see what it is worth, and
    # also the answer for a file whose grammar guesses wrong.
    m.syntax = not m.syntax
    m.rebuildDiff()
    return batch(m.scheduleHl(),
                 m.setStatus(if m.syntax: "syntax highlighting on"
                             else: "syntax highlighting off"))
  elif k.matches("c"):
    ...
    if sha.len > 0:
      return batch(diffCmd(m.root, sha, m.gitColour, m.diffGen),
                   m.setStatus(if m.gitColour: "patch coloured by git — no syntax layer"
                               elif m.syntax: "patch coloured here, syntax over it"
...
proc update(m: Model, msg: Msg): (Model, Cmd) =
  ...
  if msg of TerminalBgMsg:
    ...
    result[0].patch = d.patch
    (result[0].patchFiles, result[0].rows) = parsePatch(d.patch)
    # The highlight on hand describes the commit that was on screen a moment
    # ago, and its file indices are about to mean different files.
    result[0].hl.setLen 0
    result[0].hlOf = ""
    ...
    result[1] = result[0].scheduleHl()

  elif msg of HlMsg:
    # Two guards, and they are not the same one: the generation drops a load the
    # reader has moved on from, and the sha drops one that arrives after a
    # `reload` has put the same generation on a different commit.
    let h = HlMsg(msg)
    if h.gen != m.hlGen or h.sha != m.diffOf: return
    result[0].hl = h.files
    result[0].hlOf = h.sha
    result[0].rebuildDiff()
...
proc header(m: Model): string =
  let t = m.theme
  let title = gradientText(" syntaxlog", t.ramp, Style().bold())
  let where =
...
proc diffPane(m: Model, r: Rect): string =
  ...
  let
    ...
    # Which of the three is drawing, since a patch that has not been coloured
    # yet and one whose file has no grammar look identical on screen.
    source =
      if m.gitColour: "git"
      elif not m.syntax: "bg-mixed"
      elif m.hlOf != m.diffOf: "syntax …"        # on, still loading
      elif m.anyGrammar: "syntax"
      else: "syntax —"                          # on, no grammar in this commit
  panel(RoundedBorder)
    .title(title)
...
proc helpOverlay(m: Model): string =
  ...
  const rows = [
    ...
    ("s", "syntax highlighting on / off (needs a known language)"),
...
proc selfTest() =
  ...
  proc fixture(n: int): Model =
    result = Model(root: "/tmp/repo", branch: "main", head: "abc1234",
                   theme: DefaultTheme, animate: true, gen: 1, syntax: true)
  ...
  # `ckDefault` is the normal answer, and the design it falls back to is not
  # `gitlog`'s: a foreground-only diff would fight the grammar, so the tints are
  # mixed from a stand-in ground instead and the foreground is left alone.
  var bg = fixture(1)
  bg.applyBackground Color()
  doAssert bg.addStyle.bgc.kind != ckDefault,
    "with no answer there is a stand-in to mix against"
  doAssert bg.addStyle.fgc.kind == ckDefault and bg.delStyle.fgc.kind == ckDefault,
    "and never a foreground of its own — that channel belongs to the capture"
  ...
  # --- the patch parse, which is what the syntax layer is laid on -------------
  const twoFiles = """diff --git a/app.py b/app.py
...
+kept
"""
  let (pf, pr) = parsePatch(twoFiles)
  doAssert pf.len == 2
  doAssert pf[0].oldPath == "app.py" and pf[0].newPath == "app.py",
    "the `a/` and `b/` prefixes are git's, not the repository's"
  doAssert pf[1].newPath == "notes.txt"
  proc rowWith(rows: seq[PatchRow], text: string): PatchRow =
    for r in rows:
      if r.text == text: return r
    PatchRow(file: -2)
  doAssert rowWith(pr, " def f(x):").kind == rkContext
  doAssert rowWith(pr, " def f(x):").oldNo == 1
  doAssert rowWith(pr, "-    total = x").oldNo == 2, "a removed line counts on the old side"
  doAssert rowWith(pr, "+    total = x + 1").newNo == 2, "and an added one on the new"
  doAssert rowWith(pr, " def f(x):").file == 0
  doAssert rowWith(pr, "+kept").file == 1, "the second stanza is the second file"
  # The trap: inside a hunk, `---` is a line someone deleted — deleting a line
  # that reads `--` produces exactly that. Read as a file header it would both
  # lose the line and re-point every row after it at the wrong file.
  doAssert rowWith(pr, "---").kind == rkDel
  doAssert rowWith(pr, "---").file == 1
  echo "ok — a patch parses into rows that know their file and their line number"

  # --- the merge: a capture foreground and a diff background on one run -------
  const
    pyOld = "def f(x):\n    total = x\n    return total\n"
    pyNew = "def f(x):\n    total = x + 1\n    return total\n"
  doAssert findLanguage("python") != nil, "the python grammar did not register"

  proc lineOf(m: Model, rowText: string): string =
    ## The pane line a given patch row produced. By index rather than by
    ## searching for the text, because the text is exactly what the syntax layer
    ## cuts up: `total = x + 1` is four runs with escapes between them, so it is
    ## no longer a substring of the line that draws it.
    for i, r in m.rows:
      if r.text == rowText and i < m.diff.lines.len: return m.diff.lines[i]
    ""

  var p = fixture(1)
  p.applyBackground hex"#0d1117"
  p.diffOf = "abc1234"
  p.patch = twoFiles
  (p.patchFiles, p.rows) = parsePatch(twoFiles)
  p.hl = @[FileHl(oldSide: parseBlob("app.py", pyOld),
                  newSide: parseBlob("app.py", pyNew)),
           FileHl()]                    # notes.txt: no grammar, and none needed
  p.hlOf = "abc1234"
  p.rebuildDiff()

  let added = p.lineOf("+    total = x + 1")
  doAssert added.len > 0
  # Bytes rather than dimensions, because nothing about the width of this is
  # wrong: the palette is background-only, so a foreground in this line can only
  # have come from a capture, and the background can only be the diff tint.
  doAssert "38;2;" in added, "the grammar's colour reached a changed line"
  doAssert "48;2;" in added, "and the diff tint is still under it"
  doAssert displayWidth(added) == displayWidth("+    total = x + 1"),
    "escapes cost no columns, however many of them there are"
  doAssert "38;2;" in p.lineOf(" def f(x):"), "a context line is highlighted too"
  doAssert "38;2;" notin p.lineOf("+kept"),
    "a file with no grammar falls through to the plain colouring"
  echo "ok — one run carries the capture's foreground and the diff's background"

  # `s` off is the plain patch again, byte for byte what `gitlog` draws.
  p.syntax = false
  p.rebuildDiff()
  doAssert "38;2;" notin p.lineOf("+    total = x + 1")
  doAssert "48;2;" in p.lineOf("+    total = x + 1"), "the diff tint is not the syntax layer"
  p.syntax = true

  # The guard that no dimensional assertion can see: a blob that is not the
  # thing on screen colours the right number of columns by the wrong line's
  # grammar. Here the line numbers still resolve — the stand-in is three lines
  # long — and the check is on the text.
  p.hl[0].newSide = parseBlob("app.py", "def g():\n    pass\n    pass\n")
  p.rebuildDiff()
  doAssert "38;2;" notin p.lineOf("+    total = x + 1"),
    "a source line that is not this line is not drawn"
  doAssert displayWidth(p.lineOf("+    total = x + 1")) ==
           displayWidth("+    total = x + 1")
  echo "ok — a highlight that does not match the patch line is not used"

  # A tab-indented line has to land in the same column either way, which is the
  # one place the two rendering paths could silently disagree: `Spans.add`
  # flattens a tab to one space, `TextArea` expands it to a stop.
  var tb = fixture(1)
  tb.applyBackground hex"#0d1117"
  const tabbedPatch = "diff --git a/t.py b/t.py\n--- a/t.py\n+++ b/t.py\n" &
                      "@@ -1,2 +1,2 @@\n def f():\n+\tx = 1\n"
  tb.diffOf = "abc1234"
  tb.hlOf = "abc1234"
  tb.patch = tabbedPatch
  (tb.patchFiles, tb.rows) = parsePatch(tabbedPatch)
  tb.hl = @[FileHl(newSide: parseBlob("t.py", "def f():\n\tx = 1\n"))]
  tb.rebuildDiff()
  let hlTab = tb.lineOf("+\tx = 1")
  doAssert "38;2;" in hlTab, "the tabbed line really is highlighted"
  tb.syntax = false
  tb.rebuildDiff()
  let plainTab = expandTabs(tb.lineOf("+\tx = 1"), DefaultTabStop)
  doAssert displayWidth(hlTab) == displayWidth(plainTab),
    "a highlighted tab is expanded to the same stop the pane would have used"
  echo "ok — colouring a line does not move its indentation"

  # The highlight's own generation guard, and the sha beside it: neither alone
  # is enough, since `reload` can put the live generation on another commit.
  var g = fixture(3)
  g.diffOf = "abc1234"
  g.patch = twoFiles
  (g.patchFiles, g.rows) = parsePatch(twoFiles)
  g.hlGen = 7
  let (stale, _) = update(g, HlMsg(gen: 6, sha: "abc1234", files: @[FileHl()]))
  doAssert stale.hlOf.len == 0, "a superseded highlight is dropped"
  let (wrongSha, _) = update(g, HlMsg(gen: 7, sha: "0000000", files: @[FileHl()]))
  doAssert wrongSha.hlOf.len == 0, "and so is one for another commit"
  let (live, _) = update(g, HlMsg(gen: 7, sha: "abc1234", files: @[FileHl()]))
  doAssert live.hlOf == "abc1234"
  echo "ok — a highlight is used only for the commit and the load it belongs to"
